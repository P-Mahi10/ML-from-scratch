...
void testAddBasic() {
    ...
    expectNear(name, C(1, 0, 1), (1.0f + 6.0f) + (10.0f + 6.0f)); // (7 + 16) = 23
}
...
void testMulBasic() {
    const std::string name = "mul: basic element-wise";
    Tensor A = createTestTensor(1, 2, 3, 2.0f); // [2,3,4;5,6,7]
    Tensor B = createTestTensor(1, 2, 3, 3.0f); // [3,4,5;6,7,8]
    Tensor C = mul(A, B);

    expectNear(name, C(0, 0, 0), 2.0f * 3.0f); // 6
    expectNear(name, C(0, 0, 2), 4.0f * 5.0f); // 20
    expectNear(name, C(0, 1, 1), 6.0f * 7.0f); // 42
}

void testMulShapeMismatch() {
    expectThrowsInvalid("mul: matrix shape mismatch", [] {
        Tensor A = createTestTensor(2, 2, 3, 0.0f);
        Tensor B = createTestTensor(2, 3, 2, 0.0f);
